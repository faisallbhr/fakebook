...
{{-- FOLLOWERS MODAL --}}
<div id="followers-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full bg-black bg-opacity-90">
    <div class="relative w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <div class="p-4">
                ...
                <form action="{{ url('delete-follower/'.$follower->id) }}" method="POST" class="flex justify-between border-b my-2">
                    @method('delete')
                    @csrf
                    <p>{{ $follower->name }}</p>
                    <button><i class="fa-regular fa-trash-can hover:text-primary"></i></button>
                </form>
                @endforeach
            </div>
        </div>
...
{{-- FOLLOWINGS MODAL --}}
<div id="followings-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full bg-black bg-opacity-90">
    <div class="relative w-full max-w-md max-h-full">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow">
            <!-- Modal header -->
            <div class="flex items-start justify-between mx-4 py-4 border-b rounded-t">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center " data-modal-hide="followings-modal">
                    <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                    <span class="sr-only">Close modal</span>
                </button>
            </div>
            ...
            <div class="p-4">
                ...
                <form action="{{ url('unfollow/'.$following->id) }}" method="POST" class="flex justify-between border-b my-2">
                    @method('delete')
                    @csrf
                    <p>{{ $following->name }}</p>
                    <button><i class="fa-regular fa-trash-can hover:text-primary"></i></button>
        ...
        <div class="bg-white flex gap-4 p-4">
            ...
            <div class="flex items-center justify-between w-full">
                ...
                <div class="flex gap-8">
                    <div>
                        <p>Followers</p>
                        <button data-modal-target="followers-modal" data-modal-toggle="followers-modal" class="w-full">{{ $followers->count() }}</button>
                    </div>
                    <div>
                        <p>Followings</p>
                        <button data-modal-target="followings-modal" data-modal-toggle="followings-modal" class="w-full">{{ $followings->count() }}</button>
                    </div>
                </div>
            </div>
        </div>
